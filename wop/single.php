...
$fuel = get_post_meta($post_id, '_fuel_meta_key', true);
$power = get_post_meta($post_id, '_power_meta_key', true);
$car_color = get_post_meta($post_id, 'car_color', true);
$term_country = get_the_terms($post_id, 'car_country');
$term_type = get_the_terms($post_id, 'car_type');
?>
    <main class="main-content">
        <div class="single-page-wrapper">
                    <?php if ( have_posts() ) : ?>
                        <?php while ( have_posts() ) : the_post(); ?>
                            <article id="post-<?php the_ID(); ?>" <?php post_class('single-car-item'); ?>>
                                <h1 class="page-title entry__title"><?php the_title(); ?></h1>
                                <?php if ( has_post_thumbnail() ) : ?>
                                        <?php the_post_thumbnail('full', array('class' => 'car-img-single')); ?>
                                <?php endif; ?>
                                <?php if (isset($price) && !empty($price)) : ?>
                                    <p>Car Price: <?php echo $price; ?> $</p>
                                <?php endif; ?>
                                <?php if (isset($fuel) && !empty($fuel)) : ?>
                                    <p>Car Fuel type: <?php echo $fuel; ?></p>
                                <?php endif; ?>
                                <?php if (isset($power) && !empty($power)) : ?>
                                    <p>Car Power: <?php echo $power; ?></p>
                                <?php endif; ?>
                                <?php if (isset($car_color) && !empty($car_color)) : ?>
                                    <div class="car-color-list"><p>Car color:</p>
                                        <svg class="car-svg" fill="<?php echo $car_color; ?>" height="40px" width="40px" version="1.1" id="Capa_1"
...
<g>
    ...
    <path d="M440,291.5c-4.418,0-8,3.582-8,8c0,8.822-7.178,16-16,16s-16-7.178-16-16s7.178-16,16-16c4.418,0,8-3.582,8-8s-3.582-8-8-8
		c-17.645,0-32,14.355-32,32s14.355,32,32,32s32-14.355,32-32C448,295.082,444.418,291.5,440,291.5z"/>
                                ...
                                <div class="car-single-country">
                                    <h2>Country:</h2>
                                    <?php foreach($term_country as $term_country_item) :?>
                                    <strong><?php  echo $term_country_item->name; ?></strong>
                                        <?php endforeach;?>
                                </div>
                                <div class="car-single-type">
                                    <h2>Car brand</h2>
                                    <?php foreach($term_type as $term_type) :?>
                                        <strong><?php  echo $term_type->name; ?></strong>
                                    <?php endforeach;?>
                                </div>
                            </article>
                        <?php endwhile; ?>
